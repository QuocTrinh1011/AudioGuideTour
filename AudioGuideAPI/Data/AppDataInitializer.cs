...
public static class AppDataInitializer
{
    ...
    private static async Task SeedDemoContentAsync(AppDbContext context)
    {
        ...
        var existingTour = await context.Tours.FirstOrDefaultAsync(x => x.Name == "Đêm Vĩnh Khánh 45 phút");
    ...
    private static async Task EnsureDemoTranslationCoverageAsync(AppDbContext context)
    {
        if (!await context.Pois.AnyAsync())
        {
            return;
